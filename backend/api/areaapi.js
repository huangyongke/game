...
exports.findAreaById = function(req, res) {
	...
	Game_area.findAreasById(params.zone_id,area_id,name,function(result){
		res.send(result)
	})
}
...
exports.updateGameArea = function(req, res) {
	...
	Game_area.findGameAreaUnique(zone_id,name,function(remark){
		if(remark){
			res.send("exist")
		}else{
...
exports.addGameArea = function(req, res) {
	var zone_id = req.body.id
	var name = req.body.name;
	Game_area.findGameAreaUnique(zone_id,name,function(remark){
		if(remark){
			res.send("exist")
		}else{
			Game_area.addGameArea(zone_id,name,function(result){
				res.send('success')
			})
